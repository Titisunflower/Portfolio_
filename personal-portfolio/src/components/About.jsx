    ...
    <div className="min-h-screen px-96 flex flex-col items-center justify-center">
      ...
      <div className="w-full flex items-center justify-between md:justify-center">
        ...
        <div className="relative max-w[800px] rounded-xl bg-zinc-100 p-5 text-justify">
            ...
            <a href="/ResumeH.pdf" download="" className="w-max flex items-center gap-x-2 mt-6 rounded-full border border-gray-300 bg-red-600 px-3 py-2 font-light text-white hover:bg-red-700 transition-colors">
      ...
      <div className="mt-20 w-full flex flex-wrap items-center justify-between gap-x-7 gap-y-10">
        {aboutData.map((item, i) => (<Achievements key={i} title={item.title} amount={item.amount}>
            {item.icon}
        </Achievements>))}
      </div>
    </div>
